...
void get_module_name(struct object *object, struct node_handle *module_definition) {
	symbol_name_length = 0;
	// module_definition("module", "a", ".", "b", ";")
	struct node_handle *current_node = object->nodes + module_definition->child_index; // "module"
...
bool initialize_symbols(struct object *object) {
	...
	while (next_node_index) {
		...
		struct node_handle *current_node = object->nodes + next_node_index;
